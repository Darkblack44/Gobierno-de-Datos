...
function navegarA(vista) {
  ...
  const botonHamburguesa = document.getElementById('botonHamburguesa'); // Referencia al botón
  ...
  if (menuMovil && !menuMovil.classList.contains('hidden')) {
    ...
    if (botonHamburguesa) {
        botonHamburguesa.classList.remove('menu-abierto'); // Quita el estilo activo
        const icono = botonHamburguesa.querySelector('i');
        if (icono) {
            icono.classList.remove('fa-times'); // Quita la X
            icono.classList.add('fa-bars');     // Vuelve a poner las barras
        }
    }
  }
