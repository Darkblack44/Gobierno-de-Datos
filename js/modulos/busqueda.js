...
function renderizarTablerosFiltrados(rol, tablerosFiltrados, terminoBusqueda) {
  const claveRol = rol === 'Estudiante' ? 'estudiantes' :
                  rol === 'Administrativo' ? 'administrativos' : 'gestores';
  ...
  const idInfoResultados = `resultadosBusqueda${rol === 'Estudiante' ? 'Estudiantes' : 
                                        rol === 'Administrativo' ? 'Administrativos' : 'Gestores'}`;
  ...
  } else if (terminoBusqueda && tablerosFiltrados.length === 0) {
              ...
              <div class="tarjeta-tablero">
                <div class="p-6">
                  <div class="flex items-start justify-between mb-4">
                    <h4 class="font-bold text-gray-800 flex-1 pr-2 text-base leading-tight">${resaltarTerminoBusqueda(tablero.titulo, terminoBusqueda)}</h4>
...
function inicializarBusqueda(rol) {
  ...
  const idEntradaBusqueda = `busqueda${rol === 'Estudiante' ? 'Estudiantes' : 
                                  rol === 'Administrativo' ? 'Administrativos' : 'Gestores'}`;
  const idBotonLimpiar = `limpiarBusqueda${rol === 'Estudiante' ? 'Estudiantes' : 
                                       rol === 'Administrativo' ? 'Administrativos' : 'Gestores'}`;
