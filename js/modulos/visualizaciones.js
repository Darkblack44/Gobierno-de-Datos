...
// ==========================================
/* global mapa, esPantallaCompleta, instanciaGraficoProgramas, calcularSnapshotComunidad */
...
function generarContenidoPopup(municipio) {
  ...
  if (api) {
    ...
    let descripcion = '';
    if (api._descripcion) {
      const textoDesc = api._descripcion.length > 110
        ? api._descripcion.substring(0, 110) + '…'
        : api._descripcion;
      descripcion = `<div style="font-size:10px;color:#9ca3af;font-style:italic;
...
// nombre: formato sin tilde para compatibilidad con api-colombia.js
const _COORD_SEDES = {
  'Fusagasugá': { coordenadas: [4.3391, -74.3636], nombre: 'Fusagasuga' },
...
function inicializarMapa() {
  ...
  // ── Datos desde CSV vía calcularSnapshotComunidad ────────────────────
