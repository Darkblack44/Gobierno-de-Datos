...
function _renderTabComunidad() {
  ...
  const elResumen = document.getElementById('textoResumenFiltrosComunidad');
  if (elResumen) elResumen.textContent = etiqueta;
...
function _renderizarKPIsComunidad(snap, cat) {
  const kpis = [
    { id:'indKpiTotal', valor:snap.total,    etiqueta:cat + ' totales',   icono:'fa-users',          badge:String(snap.anio),  badgeClass:'kpi-ind-card__badge--anio', color:'verde' },
  ...
  cont.innerHTML = kpis.map(k => `
    <div class="kpi-ind-card kpi-ind-card--${k.color}">
      ...
      <div class="kpi-ind-card__top">
        ...
        <span class="kpi-ind-card__badge ${k.badgeClass || ''}">${k.badge}</span>
      </div>
...
function _crearGraficoApilado(planta) {
  ...
  // Ordenar de mayor a menor por la suma de los tres componentes apilados
  const ordenado = [...planta].sort((a, b) =>
    (b.m2Aulas + b.m2Laboratorios + b.m2Deportivo) -
    (a.m2Aulas + a.m2Laboratorios + a.m2Deportivo)
  );
  ...
  graficoApilado = new Chart(ctx, {
    ...
    data: {
      labels: ordenado.map(r => _displaySede(r.sede)),
      datasets: [
        { label: 'M² Aulas',        data: ordenado.map(r => r.m2Aulas),        backgroundColor: _PAL_INFRA[0], borderRadius: 0 },
        { label: 'M² Laboratorios', data: ordenado.map(r => r.m2Laboratorios), backgroundColor: _PAL_INFRA[1], borderRadius: 0 },
        { label: 'M² Deportivo',    data: ordenado.map(r => r.m2Deportivo),    backgroundColor: _PAL_INFRA[2], borderRadius: 2 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
  ...
  cont.innerHTML = planta.map(c => {
    ...
    <div class="tarjeta-campus">
      ...
      <div class="tarjeta-campus-cuerpo">
        ...
        <p class="fuente-datos mt-3">Fuente: SNIES</p>
      </div>
    </div>
  `;
  }).join('');
