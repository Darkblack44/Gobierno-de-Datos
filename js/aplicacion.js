...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Nota de desarrollo: si los CSVs no se cargan (protocolo file://) los KPIs muestran –
  // Para ver datos: ejecutar iniciar.bat o cualquier servidor local (python -m http.server 8000)
  if (getEstadoCargaIndicadores() === 'error') {
    console.warn(
      '⚠ GOBIERNO DE DATOS: CSV no disponible (probablemente file://).\n' +
      '  Para ver datos: ejecuta iniciar.bat o abre con Live Server.'
    );
  }
  ...
  // Pre-inicializar contadores de tableros (evita que muestren "-" antes de navegar)
  actualizarEstadisticasRol('Estudiante');
  actualizarEstadisticasRol('Administrativo');
  actualizarEstadisticasRol('Gestor');
  configurarMenuMovil();
  ...
  window.addEventListener('resize', () => {
    clearTimeout(globalThis.temporizadorRedimension);
    globalThis.temporizadorRedimension = setTimeout(manejarRedimension, 400);
  });
...
const navegarAOriginal = globalThis.navegarA;
globalThis.navegarA = function(vista) {
...
function inicializarArrastreLogos() {
  ...
  const pausar = () => {
    ...
    logosContenedor.style.animation = 'none';       // detener por completo (no solo pausar) para evitar conflicto con will-change compositor
  ...
  const reanudar = (translateXFinal) => {
    ...
    logosContenedor.style.transform = '';
    logosContenedor.getBoundingClientRect(); // Forzar reflow del navegador
